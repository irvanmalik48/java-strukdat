...
class Main {
	...
	public static void main(String[] args) {
		...
		while (index < arr.length) {
			...
			if (arr[index] > 80) counter++;
			// Jumlahkan seluruh angka ganjil
			if (arr[index] % 2 == 1) sum += arr[index];
		...
		if (found == true) System.out.printf(
			"Ditemukan %d angka 100\n",
			idx.size()
		);

		if (found == true) System.out.printf(
			"Ditemukan angka 100 pada indeks ke-%d di posisi mendekati awal\n",
			idx.get(0)
		);

		// Apakah ada bilangan 100? (reversed index order)
		if (found == true) System.out.printf(
			"Ditemukan angka 100 pada indeks ke-%d di posisi mendekati akhir\n",
			idx.get(idx.size() - 1)
		);
		...
		} else System.out.print("Tidak ditemukan angka 100\n");
	}
}
